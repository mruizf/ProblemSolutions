...
# Problem description
 Given a quantity change ``$` n `$``, we want to know how many combinations we can obtain to give this change using coins with ``$\{s_1 ,s_ 2,\ldots, s_m\}$`` values.$$\sum_{i=0}^n i^2 = \frac{(n^2+n)(2n+1)}{6}$$.
